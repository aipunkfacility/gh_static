function PopularSection(excursions, services, transport) {
  const popularItems = [
      ...
      <div class="max-w-6xl mx-auto px-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ${popularItems.map(item => {
            if (item.type === 'excursion') return renderCardService({title: item.title, shortDescription: item.shortDescription, slug: item.slug});
            if (item.type === 'transport') return renderCardTransport(item);
